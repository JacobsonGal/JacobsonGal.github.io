...
function syncStylesheets(page) {
  const version = 'rightclick-menu-8';
...
async function mountPage(page) {
  if (page === 'home') {
    const { mountHomePage } = await import('./app.js?v=rightclick-menu-8');
    await mountHomePage();
  } else {
    const { mountResumePage } = await import('./resume-page.js?v=rightclick-menu-8');
...
async function destroyPage(page) {
  if (page === 'home') {
    const { destroyHomePage } = await import('./app.js?v=rightclick-menu-8');
    destroyHomePage();
  } else if (page === 'resume') {
    const { destroyResumePage } = await import('./resume-page.js?v=rightclick-menu-8');
    destroyResumePage();
  }
}
