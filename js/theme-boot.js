(function bootTheme() {
  ...
  const themeLock = document.querySelector('meta[name="theme-lock"]')?.content;
  const appearanceLock = document.querySelector('meta[name="appearance-lock"]')?.content;
  let theme = themeLock === 'arctic' || themeLock === 'editorial' ? themeLock : 'arctic';
  ...
  let appearance = appearanceLock === 'light' || appearanceLock === 'dark'
    ? appearanceLock
